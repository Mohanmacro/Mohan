...
(function ($) {
    ...
    class $mol_build extends $.$mol_object {
        ...
        tsPaths({ path, exclude }) {
            return this.sourcesAll({ path, exclude }).filter(src => /tsx?$/.test(src.ext())).map(src => src.path());
        }
        tsCompile({ path, exclude }) {
            const paths = this.tsPaths({ path, exclude });
            if (!paths.length)
                return null;
            var host = $node.typescript.createWatchCompilerHost(paths, this.tsOptions(), Object.assign({}, $node.typescript.sys, { setTimeout: (cb) => cb(), writeFile: (path, content) => {
